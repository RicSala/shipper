export function hexToHSL(H) {
    ...
    let a = isNaN(parseFloat(H.substring(7, 9))) ? 1 : Math.round(parseFloat(H.substring(7, 9)) / 255 * 10000) / 10000;
...
export function generateColorVariables(colorsToGenerate, shades) {
    ...
    colorsToGenerate.forEach(colorName => {
        shades.forEach(shade => {
            lightTheme[`--${colorName}-${shade}`] = `var(--light-${colorName}-${shade})`;
            darkTheme[`--${colorName}-${shade}`] = `var(--dark-${colorName}-${shade})`;
        });
    });
